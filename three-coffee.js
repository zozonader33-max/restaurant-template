...
const camera=new THREE.PerspectiveCamera(35,1,.1,100);camera.position.set(0,1.8,7);
...
function animate(t){const reduced=matchMedia("(prefers-reduced-motion: reduce)").matches;cup.rotation.y=reduced ? .35 : Math.sin(t*.00055)*.14+pointer;cup.rotation.x=.1;cup.position.y=reduced?0:Math.sin(t*.001)*.08;cup.children.filter(x=>x.userData.offset!==undefined).forEach(s=>{s.position.y=1.55+Math.sin(t*.001+s.userData.offset)*.08});renderer.render(scene,camera);requestAnimationFrame(animate)}requestAnimationFrame(animate);
